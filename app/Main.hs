...
compileSource opts buildDir src isRepl = runCompilation $ do
  ...
  parsedAST <- case parsedEither of
    Right ast -> do
      ...
      return ast
    Left err -> do
      let errMsg = errorBundlePretty (err :: ParseErrorBundle T.Text Void)
      logErrorN $ "Program parsing failed: " <> T.pack errMsg
      liftIO exitFailure
...
--           result <- runStderrLoggingT $ do
--             parsedEither <- parseProgram inputText
--             case parsedEither of
--               Left err -> return $ Left $ errorBundlePretty (err :: ParseErrorBundle T.Text Void)
--               Right ast ->
