...
void gc_mark_obj(struct gc_context *ctx, struct obj *obj) {
  obj->mark = BLACK;
  gc_func_map[obj->tag].zyn_mark(obj, ctx);
}
...
void zyn_gc_minor(struct gc_context *ctx, struct thunk *thnk) {
...
{
  if (ck_array_init(&zyn_mutators, CK_ARRAY_MODE_SPMC, &my_allocator, 10) == 0) {
...
{
  pthread_mutex_lock(&mutators_lock);
  if (!ck_array_remove(&zyn_mutators, (void *)thd)) {
...
int gc_is_mutator_active(gc_thread_data * thd)
{
  ...
  gc_thread_data *m;
  CK_ARRAY_FOREACH(&zyn_mutators, &iterator, &m) {
    if (m == thd) {
      return 1;
    }
  }
...
//zyn_display(thd, old_obj, stderr);
//fprintf(stderr, "\n");
...
void gc_wait_handshake()
{
  ...
  CK_ARRAY_FOREACH(&zyn_mutators, &iterator, &m) {
    while (1) {
      ...
      if (thread_status == ZYN_THREAD_STATE_BLOCKED ||
          thread_status == ZYN_THREAD_STATE_BLOCKED_COOPERATING) {
...
//printf("DEBUG - is mutator still blocked?\n");
          ...
          if (ck_pr_cas_int((int *)&(m->thread_state),
                            ZYN_THREAD_STATE_BLOCKED,
                            ZYN_THREAD_STATE_BLOCKED_COOPERATING)
              ||
...
//printf("DEBUG - update mutator GC status\n");
      ...
      } else if (thread_status == ZYN_THREAD_STATE_TERMINATED) {
...
void gc_mark_globals(object globals, object global_table)
{
  ...
  fprintf(stderr, "zyn_global_variables %p\n", globals);
#endif
...
void gc_mutator_thread_blocked(gc_thread_data * thd, object cont)
{
  ...
  if (!ck_pr_cas_int((int *)&(thd->thread_state),
                     ZYN_THREAD_STATE_RUNNABLE, ZYN_THREAD_STATE_BLOCKED)) {
    fprintf(stderr,
                                ...
                                object maybe_copied)
{
  ...
  if (!ck_pr_cas_int((int *)&(thd->thread_state),
                     ZYN_THREAD_STATE_BLOCKED, ZYN_THREAD_STATE_RUNNABLE)) {
...
//printf("DEBUG - Call into gc_cont after collector coop\n");
    ...
    if (type_of(thd->gc_cont) == pair_tag || prim(thd->gc_cont)) {
      thd->gc_args[0] = result;
      zyn_apply_from_buf(thd, 2, thd->gc_cont, thd->gc_args);
    } else {
      object buf[1] = { result };
